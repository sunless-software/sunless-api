...
e.end_date, 'created_at', e.created_at, 'updated_at', e.updated_at) order by e.start_date desc) from experiences e where e.user_id = u.id), '[]') as experiences, coalesce(json_agg(distinct 
...
left join educations ed on ed.user_id = u.id where u.id = $1 and ($2::boolean IS TRUE OR u.public = TRUE) and 
