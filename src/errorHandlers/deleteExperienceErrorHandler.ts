...
export default function deleteExperienceErrorHandler(
  err: CustomError,
  req: Request,
  res: Response
) {
  if (isPgError(err.error)) {
    logger.error(
      `The following unexpected postgres error has occurred while trying to delete a experience: `
    );
    logger.error(err);
  ...
  logger.error(
    "The following unexpected error has occurred while trying to delete a experience: "
  );
  logger.error(err);
  sendResponse(DEFAULT_ERROR_API_RESPONSE, res);
  return;
